...
class distortionNode extends AudioWorkletNode {
  ...
  pitchWheel(channel, wheel) {
    for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
      ...
      this.setParamValue(
        pw.path,
        distortionNode.remap(wheel, 0, 16383, pw.min, pw.max)
      );
      if (this.output_handler) {
  ...
  async load() {
    try {
      ...
      if (this.fWorkletProcessors.indexOf(name) === -1) {
        try {
          let re = /JSON_STR/g;
          let distortionProcessorString1 = distortionProcessorString.replace(
            re,
            json
          );
          let real_url = window.URL.createObjectURL(
            new Blob([distortionProcessorString1], { type: "text/javascript" })
          );
      ...
      this.node.onprocessorerror = () => {
        console.log("An error from distortion-processor was detected.");
      };
      return this.node;
    } catch (e) {
      console.error(e);
